...
En un mundo cada vez más digitalizado, las empresas e-commerce como **Olist** en Brasil, enfrentan desafíos constantes para mantenerse competitivas y satisfacer las demandas en evolución de los consumidores. En este contexto, el análisis de datos emerge como un recurso invaluable que no solo proporciona una comprensión profunda del comportamiento del cliente, sino que también impulsa decisiones estratégicas fundamentadas en información precisa y oportuna.

El presente proyecto de análisis de datos para **Olist** tiene como objetivo capitalizar la riqueza de información generada a través de las interacciones de los usuarios en su plataforma digital. Desde la identificación de patrones de compra hasta la optimización de la experiencia del cliente, este proyecto se centra en aprovechar los datos como un activo estratégico para impulsar el crecimiento empresarial y mejorar la competitividad en el mercado e-commerce.

---

## Objetivo

Desarrollar una solución que permita a los gerentes y directivos de la compañía **Olist** dar seguimiento a las compras y productos que realizan sus clientes a través de una aplicación dinámica e interactiva que permita la mejora en la toma de decisiones, la mejora continua y el conocimiento de las preferencias de compra.

Previo al desarrollo de esta solución, investigaremos más sobre los datos que nos proporciona **Olist** en el siguiente notebook que se conecta directamente con un **Cubo OLAP** de la empresa olist.

Puedes ver el desarrollo del proyecto dando scroll al mouse, o si prefieres, puedes ir a los resultados directamente haciendo clic en el siguiente enlace.

[Ir a los resultados](#end)

---

## Tabla de Contenido

1. [Importar Librerias](#librerias)
2. [Cargar los Datos](#cargar)
3. [Limpieza de Datos](#limpieza)
    
    3.1 Verificar errores en los datos

    3.2 Revisión de valores ausentes

    3.3 Revisión de valores duplicados

4. [EDA - Análisis Exploratorio de Datos](#analisis)
    
    4.1 Requerimientos marketing

    4.2 Requerimientos finanzas

5. [Análisis Estadístico de Datos](#estadistica)
...
6. [Análisis Estocástico](#estocastic)
...
7. [Conclusiones y Reporte](#conclusion)
...
clientes_compra = df['customer_code'].nunique()
clientes_compra

recuento_ordenes = df['order_status'].value_counts()
recuento_ordenes
...
ciudades_clientes = df['customer_city'].describe()
ciudades_clientes
...
top_10.rename(columns={'customer_city':'ciudades'}, inplace=True)
top_10

df_top_10 = pd.concat([top_10, df_otros_gpd])
df_top_10 = df_top_10.reset_index(drop=True)
df_top_10.rename(columns={'customer_code':'no_clientes'}, inplace=True)
df_top_10
...
productos_totales = df['product_category_name'].nunique()
productos_totales
...
pagos = finance_df['payment_type'].value_counts()
pagos
...
fig = plt.subplots(figsize=(20,45))
...
"""Esta gráfica nos muestra la frecuencia de compra de los clientes por día, se puede observar que aproximadamente cada fin de semana los clientes acuden a realizar una compra. El promedio de compras podemos encontrarlo en la línea amarilla mientras que en el color verde podemos encontrar la desviación estándar de este promedio de compra.
...
"""En estas gráficas podemos observar las 3 características principales del proceso estocástico:

- Tendencia.- puede observarse en la segunda gráfica de arriba hacia abajo.

- Frecuencia - puede visualizarse en el tercer gráfico de arriba hacia abajo.

- Ruido - Simplemente son valores que sobran en nuestro gráfico y en realidad no nos indican mucho, puedes encontrarlo en el cuarto gráfico de arriba hacia abajo.
...
"""El Test de Dickey-Fuller es una herramienta estadística utilizada para evaluar si una serie temporal tiene raíces unitarias, es decir, si muestra evidencia de ser estacionaria o no estacionaria.

La estacionariedad es una propiedad importante en el análisis de series temporales, ya que implica que las propiedades estadísticas de la serie, como la media y la varianza, permanecen constantes en el tiempo.
"""
...
<a name='conclusion'></a>
## 7 Conclusiones y Reporte Final

Después de pasar un tiempo analizando nuestros datos, se encontraron los puntos más importantes para desarrollar una solución adecuada para el cliente.

- **Requerimientos de Marketing** encontramos algunos insights que pueden ser de utilidad para dar seguimiento a las preferencias del cliente, los productos más comprados y aquellos que son los más valorados.

- **Requerimientos de Finanzas** también encontramos insights interesantes que nos dicen cuáles son los productos más comprados en ciertos periodos como año y mes, cuáles son los productos que más ingresos generan, así como los medios de pago más utilizados por los clientes.

En la siguiente sección puedes encontrar un reporte completo sobre los resultados finales.

**ACCESO A PLATAFORMA INTERACTIVA** - [clic aquí](https://public.tableau.com/views/BodegaSpeedy/DashboardBodegaSpeedy?:language=en-US&:sid=&:display_count=n&:origin=viz_share_link)
"""

# @title
print('REPORTE FINAL OLIST BRASIL')
print('-'*50)
print('-'*50)
print('')

print('No DE CLIENTES QUE HAN REALIZADO UNA COMPRA:')
print('-'*40)
print(clientes_compra)
print('')

print('RECUENTO DE ORDENES EN PROCESO:')
print('-'*40)
print(recuento_ordenes)
print('')

print('PORCENTAJE DE CIUDADES CON MÁS CLIENTES:')
print('-'*40)
print(x)
print('')

print('No DE PRODUCTOS ÚNICOS QUE OFRECE LA COMPAÑÍA')
print('-'*40)
print(productos_totales)
print('')

print('TOP 10 PRODUCTOS MEJOR EVALUADOS')
print('-'*40)
print(top_10_prod)
print('')

print('PRODUCTOS MÁS VENDIDOS EN 2018')
print('-'*40)
print(total_sells.head(10))
print('')

print('MÉTODO DE PAGO MÁS UTILIZADO')
print('-'*40)
print(pagos)
print('')